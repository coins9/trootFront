...
interface Props {
  visible: boolean;
  onClose: () => void;
  onPurchase: (plan: CardAdPlan) => void;
}
...
const CardAdBottomSheet = memo(({ visible, onClose, onPurchase }: Props) => {
  ...
  const daySavingHint = useCallback((plan: CardAdPlan) => {
...
CardAdBottomSheet.displayName = 'CardAdBottomSheet';
export default CardAdBottomSheet;
